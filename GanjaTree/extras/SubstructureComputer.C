  ...
  SubstructureComputer( int activeAreaRepeats = 1,
	...
	void operator()  (size_t np, float* E, float *px, float *py, float *pz,
		...
		for(unsigned ip=0; ip < np; ++ip) {
			...
			float p2 = pj.pt2();
			float eta = pj.eta() - jeta;
			float phi = pj.phi() - jphi;
			sum_pt       += pj.pt();
			sum_weight   += p2;
			sum_deta     += eta*p2;
			sum_dphi     += phi*p2;
		...
		if(sum_weight > 0){
			...
			/// std::cerr << "cmpt axes " << ave_deta << " " << ave_dphi
			/// 	  << " " << ave_deta2
			/// 	  << " " << ave_dphi2
			/// 	  << " " << a << " " << b << " " << c
			/// 	  << std::endl; 
		...
		axis2 = (a+b-delta > 0 ?  sqrt(0.5*(a+b-delta)) : 1.e-3);
		axis1 = (a+b+delta > 0 ?  sqrt(0.5*(a+b+delta)) : 1.e-4);
